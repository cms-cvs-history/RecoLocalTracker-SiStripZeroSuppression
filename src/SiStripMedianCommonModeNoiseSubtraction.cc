...
void SiStripMedianCommonModeNoiseSubtraction::subtract(const uint32_t& detId,std::vector<int16_t>& digis){
  ...
  std::vector<int16_t>::iterator mid = APVdigis.begin() + 64;
  ...
  for (int iAPV=0; iAPV<nAPV; iAPV++){
    ...
    std::vector<int16_t>::iterator i63 = std::max_element(APVdigis.begin(), mid);
    CM = (*mid + *i63)/2.;
